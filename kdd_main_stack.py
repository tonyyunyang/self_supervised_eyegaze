...
from modules.kdd_model import kdd_model4finetune_convstack, kdd_model4pretrain, kdd_model4finetune, kdd_model4pretrain_convstack
...
def main():
    ...
    if config["general"]["pretrain_model"] is None:
        ...
        model = kdd_model4pretrain_convstack(config, feat_dim)
    ...
    model = kdd_model4finetune_convstack(config, feat_dim, num_classes)
    loss = hyperparameters.loss
    optimizer = hyperparameters.optimizer(model.parameters(), hyperparameters.lr,
                                          weight_decay=hyperparameters.weight_decay)
