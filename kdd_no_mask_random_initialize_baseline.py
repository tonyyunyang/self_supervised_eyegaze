...
from modules.kdd_model import kdd_model4fullysupervise_finetune, kdd_model4fullysupervise_pretrain, kdd_model4fullysupervise_random_initialize, kdd_model4pretrain, kdd_model4finetune
from modules.pretrain_hyperparameters import KDD_Fully_Supervised_Pretrain_Hyperparameters, KDD_Pretrain_Hyperparameters
from utils.finetune import finetune_kdd_model, eval_finetune_kdd_model, fully_supervise_train_kdd_model_finetune, fully_supervise_train_kdd_model_pretrain, fully_supervise_train_kdd_model_random_initialize
...
def main():
    ...
    model = kdd_model4fullysupervise_random_initialize(config, feat_dim, num_classes)
    loss = hyperparameters.loss
    optimizer = hyperparameters.optimizer(model.parameters(), hyperparameters.lr,
                                          weight_decay=hyperparameters.weight_decay)

    # eyegaze_data_loader[1] is the training set, and eyegaze_data_loader[2] is the validation set
    fully_supervise_train_kdd_model_random_initialize(model, loss, optimizer, eyegaze_data_loader[2], eyegaze_data_loader[3], config)
